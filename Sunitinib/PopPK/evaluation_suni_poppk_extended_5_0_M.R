...
data<-read.nm.tables("Suni_focei_sim_split_5.tab")
...
write.csv(result_df_a, file = sprintf("mean_ipred_values_sim_focei_suni_parent_%d.csv", m), row.names = FALSE)
...
ggsave(sprintf("dv_ipred_plot_test_focei_suni_parent_%d.jpg", m), width=6, height=6)


#SAEM -------------------------------------------------------------------------
data<-read.nm.tables("Suni_saem_sim_split_5.tab")
...
# Save the result data frame to a CSV file
write.csv(result_df_b, file = sprintf("mean_ipred_values_sim_saem_suni_parent_%d.csv", m), row.names = FALSE)
...
ggsave(sprintf("dv_ipred_plot_test_saem_suni_parent_%d.jpg", m), width=6, height=6)
...
# Save the result data frame to a CSV file
write.csv(result_df_c, file = sprintf("mean_ipred_values_sim_focei_suni_metabolite_%d.csv", m), row.names = FALSE)
...
ggsave(sprintf("dv_ipred_plot_test_focei_suni_metabolite_%d.jpg", m), width=6, height=6)
...
# Save the result data frame to a CSV file
write.csv(result_df_d, file = sprintf("mean_ipred_values_sim_saem_suni_metabolite_%d.csv", m), row.names = FALSE)
...
ggsave(sprintf("dv_ipred_plot_test_saem_suni_metabolite_%d.jpg", m), width=6, height=6)
...
# save results
write.csv(quality, file = sprintf("quality_measures_test_focei_parent_%d.csv", m), row.names = FALSE)
...
# save results
write.csv(quality, file = sprintf("quality_measures_test_saem_parent_%d.csv", m), row.names = FALSE)
...
# save results
write.csv(quality, file = sprintf("quality_measures_test_focei_metabolite_%d.csv", m), row.names = FALSE)
...
# save results
write.csv(quality, file = sprintf("quality_measures_test_saem_metabolite_%d.csv", m), row.names = FALSE)
...
# FOCE-I  ----------------------------------------------------------------------
data_cwres<- read.nm.tables("Suni_focei_split_5.tab")
...
ggsave(sprintf("cwres_pred_plot_train_focei_suni_parent_%d.jpg", m), width=6, height=6)
...
ggsave(sprintf("cwres_time_plot_train_focei_suni_parent_%d.jpg", m), width=6, height=6)


# SAEM -------------------------------------------------------------------------
data_cwres<- read.nm.tables("Suni_saem_split_5.tab")
data_cwres <- subset(data_cwres, CMT == 2) # parent
data_cwres$IPRED <- as.numeric(data_cwres$IPRED)
data_cwres$EXPIPRED <- as.numeric(data_cwres$EXPIPRED)
data_cwres$CWRES <- as.numeric(data_cwres$CWRES)
...
cwres_pred<- ggplot(data_cwres,aes(x=EXPIPRED,y=CWRES))+
  ...
  scale_x_continuous(limits=c((min(data_cwres$EXPIPRED)-1),(max(data_cwres$EXPIPRED)+1))) +
  scale_y_continuous(limits=c((min(data_cwres$CWRES)-1),(max(data_cwres$CWRES)+1)))+
  ggtitle ("Conditional Weight Residual Errors (CWRES) vs. Individual Predicted Concentrations \n Method: SAEM-I (train data) Parent [5]") +
  theme(plot.title = element_text(size=10.5, hjust=0.5)) + # Set theme
  ylab("Conditional Weight Residual Error (CWRES)")+ xlab("Individual Predicted Concentration [ng/mL]")
cwres_pred

ggsave(sprintf("cwres_pred_plot_train_saem_suni_parent_%d.jpg", m), width=6, height=6)
...
ggsave(sprintf("cwres_time_plot_train_saem_suni_parent_%d.jpg", m), width=6, height=6)
...
cwres_pred<- ggplot(data_cwres,aes(x=EXPIPRED,y=CWRES))+
  ...
  scale_x_continuous(limits=c((min(data_cwres$EXPIPRED)-1),(max(data_cwres$EXPIPRED)+1))) +
  scale_y_continuous(limits=c((min(data_cwres$CWRES)-1),(max(data_cwres$CWRES)+1)))+
  ggtitle ("Conditional Weight Residual Errors (CWRES) vs. Individual Predicted Concentrations \n Method: FOCE-I (train data) SU12662 [5]") +
  theme(plot.title = element_text(size=10.5, hjust=0.5)) + # Set theme
  ylab("Conditional Weight Residual Error (CWRES)")+ xlab("Individual Predicted Concentration [ng/mL]")
cwres_pred

ggsave(sprintf("cwres_pred_plot_train_focei_suni_metabolite_%d.jpg", m), width=6, height=6)
...
cwres_time<- ggplot(data_cwres,aes(x=TIME,y=CWRES))+
  ...
  scale_y_continuous(limits=c((min(data_cwres$CWRES)-1),(max(data_cwres$CWRES)+1)))+
  geom_abline(intercept = 0, slope = 0,size=0.5)+ # Add line of unity
  ggtitle ("Conditional Weight Residual Errors (CWRES) vs. Time [5] \n Method: FOCE-I (train data) SU12662 [5]") +
  theme(plot.title = element_text(size=12, hjust=0.5)) + # Set theme
  ylab("Conditional Weight Residual Error (CWRES)")+ xlab("Time [h]")
cwres_time

ggsave(sprintf("cwres_time_plot_train_focei_suni_metabolite_%d.jpg", m), width=6, height=6)
...
cwres_pred<- ggplot(data_cwres,aes(x=EXPIPRED,y=CWRES))+
  ...
  scale_x_continuous(limits=c((min(data_cwres$EXPIPRED)-1),(max(data_cwres$EXPIPRED)+1))) +
  scale_y_continuous(limits=c((min(data_cwres$CWRES)-1),(max(data_cwres$CWRES)+1)))+
  ggtitle ("Conditional Weight Residual Errors (CWRES) vs. Individual Predicted Concentrations \n Method: SAEM-I (train data) SU12662 [5]") +
  theme(plot.title = element_text(size=10.5, hjust=0.5)) + # Set theme
  ylab("Conditional Weight Residual Error (CWRES)")+ xlab("Individual Predicted Concentration [ng/mL]")
cwres_pred

ggsave(sprintf("cwres_pred_plot_train_saem_suni_metabolite_%d.jpg", m), width=6, height=6)
...
ggsave(sprintf("cwres_time_plot_train_saem_suni_metabolite_%d.jpg", m), width=6, height=6)

# Section 4 :  ETA distribution  -----------------------------------------------

#FOCE-I ------------------------------------------------------------------------

data_etas <-read.nm.tables("etas_Suni_focei_split_5.tab")
...
eta_plot <-ggplot(data_etas, aes(x=ETA1sc)) +geom_histogram(aes(y = after_stat(density)), binwidth=0.02, col = "black") +  
  ...
  labs(
    title = "ETA Distribution (IIV V2) [5] \n Method: FOCE-I (train data)",
    x = "ETA",
    y = "Density"
  ) +
  theme(plot.title = element_text(size=11, hjust=0.5)) + # Set theme
...
ggsave(sprintf("eta_distribution_plot_v2_focei_%d.jpg",m), width= 6, height= 6)
...
eta_plot <-ggplot(data_etas, aes(x=ETA2sc)) +geom_histogram(aes(y = after_stat(density)), binwidth=0.02, col = "black") +  
  geom_density(alpha = 1, col= "#004a9f") +
  labs(
    title = "ETA Distribution (IIV V3) [5] \n Method: FOCE-I (train data)",
    x = "ETA",
    y = "Density"
  ) +
  theme(plot.title = element_text(size=11, hjust=0.5)) + # Set theme
...
ggsave(sprintf("eta_distribution_plot_v3_focei_%d.jpg",m), width= 6, height= 6)
...
eta_plot <-ggplot(data_etas, aes(x=ETA3sc)) +geom_histogram(aes(y = after_stat(density)), binwidth=0.02, col = "black") +  
  geom_density(alpha = 1, col= "#004a9f") +
  labs(
    title = "ETA Distribution (IIV CLP) [5] \n Method: FOCE-I (train data)",
    x = "ETA",
    y = "Density"
  ) +
  theme(plot.title = element_text(size=11, hjust=0.5)) + # Set theme
...
ggsave(sprintf("eta_distribution_plot_clp_focei_%d.jpg",m), width= 6, height= 6)
...
eta_plot <-ggplot(data_etas, aes(x=ETA4)) +geom_histogram(aes(y = after_stat(density)), binwidth=0.05, col = "black") +  
  geom_density(alpha = 1, col= "#004a9f") +
  labs(
    title = "ETA Distribution (IIV FM) [5] \n Method: FOCE-I (train data)",
    x = "ETA",
    y = "Density"
  ) +
  theme(plot.title = element_text(size=11, hjust=0.5)) + # Set theme
...
ggsave(sprintf("eta_distribution_plot_fm_focei_%d.jpg",m), width= 6, height= 6)


# SAEM -------------------------------------------------------------------------

data_etas <-read.nm.tables("etas_Suni_saem_split_5.tab")
...
ggsave(sprintf("eta_distribution_plot_v2_saem_%d.jpg",m), width= 6, height= 6)
...
ggsave(sprintf("eta_distribution_plot_v3_saem_%d.jpg",m), width= 6, height= 6)
...
ggsave(sprintf("eta_distribution_plot_clp_saem_%d.jpg",m), width= 6, height= 6)
...
ggsave(sprintf("eta_distribution_plot_fm_saem_%d.jpg",m), width= 6, height= 6)
