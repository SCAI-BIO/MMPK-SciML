...
setwd("C:/Users/teply/Documents/5FU_NONMEM_final/vpc_paper_all")
...
files <- list.files(pattern = "9.npctab.dta", recursive = TRUE, include.dirs = TRUE)
...
data <- read.csv("Suni_PK_final_raw.csv")# for DV vs IPRED and other GOF
...
## Search for the generated file by PsN in the sub-folders
files <- list.files(pattern = "15.npctab.dta", recursive = TRUE, include.dirs = TRUE)
...
######### Read dataset with original observations
data_5fu <- read.csv("corrected_NM_Data_final_clean.csv", sep=";")
