########################################################################################################################
# XGBoost, LightGBM for Sunitinib Project                                                                              #
...
def optimize_xtreme_gradient_boosting_hyperparameters(split, data_augmentation,
    ...
    def objective(trial):
        X_train, y_train, X_test, y_test = Data_load_clean_suni.train_test_split(
            split=split,
        ...
        csv_filename = f'suni_tuning_mse_{algorithm_name}_split_{split}_augmentation_{data_augmentation}.csv'
...
def run_xtreme_gradient_boosting_experiment(split, data_augmentation,
    ...
    for i in tqdm(range(num_epochs), desc="Epochs"):
        ...
        csv_filename = f'suni_params_test_{algorithm_name}_split_{split}_augmentation_{data_augmentation}.csv'
        ...
        results_df.to_csv(
            f'suni_results_{algorithm_name}_split_{split}_augmentation_{data_augmentation}.csv',
            index=False)
